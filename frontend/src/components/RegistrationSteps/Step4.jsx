...
export default function Step4({ data, handleChange, addArrayItem, removeArrayItem }) {
  ...
  return (
    <div className="space-y-6">
          ...
          {(data.awards || []).length > 0 ? (
            data.awards.map((award, index) => (
              <div key={index} className="space-y-4 border p-4 rounded-lg">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Upload Award Certificate:</label>
                  {award.file && (
      ...
      <fieldset className="pt-4 border-t border-gray-200">
        <legend className="block text-sm font-medium text-gray-700 mb-2">
          Has your company established any collaborations with other institutions?
        </legend>
        <div className="flex items-center gap-x-6">
          <div className="flex items-center">
            <input
