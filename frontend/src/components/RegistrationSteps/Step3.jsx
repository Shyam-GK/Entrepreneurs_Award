...
export default function Step3({ data, handleChange, addArrayItem, removeArrayItem }) {
  // Ensure at least one merger block
  const mergers = data.hasMergers === 'true' && data.mergers && data.mergers.length > 0 ? data.mergers : [{}];
    ...
    <div className="space-y-6">
      ...
      <fieldset className="pt-4 border-t border-gray-200">
        <legend className="block text-sm font-medium text-gray-700 mb-2">
          Has your organization undergone any merger or acquisition involving a corporate or multinational organization?
        </legend>
        ...
        <div className="space-y-6 pt-4 border-t border-gray-200 mt-6">
          ...
          {mergers.map((merger, index) => (
            <div key={index} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Name of the Corporate/Multinational Organization Involved:</label>
                <input
                  type="text"
                  name="mergerCompany"
